...
window.addEventListener('DOMContentLoaded', () => {
  const modalTimerId = setTimeout(() => Object(_modules_modal__WEBPACK_IMPORTED_MODULE_3__["openModal"])('.modal', modalTimerId), 50000); // открывать модал через N секунд
  ...
  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_6__["default"])('.timer', '2022-08-20');
});
...
function slider({
  ...
  letTotalSlideIndex();
  letCurrentSlideIndex(); // Устанавливаем ширину окна слайдера, в зависимости от количества слайдов на странице. + анимация

  slidesField.style.width = 100 * slides.length + '%'; // width 400% 

  slidesField.style.display = 'flex';
  slidesField.style.transition = "0.5s all"; // скрываем слайды вне зоны видипости

  slidesWrapper.style.overflow = 'hidden'; // Устанавливаем для каждого слайда одинаковую ширину. (Проверка на разыне картинки)
  ...
  slider.append(indicators); // Создаем точки по количеству слайдов на странице. Задаем класс со стилями. 
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  const tabs = document.querySelectorAll(tabsSelector),
        ...
        tabsParants = document.querySelector(tabsParentSelector); // Скрываем все табы 
...
async function getResource(url) {
  let res = await fetch(url); // Добавляем проверку и сообщение об ошибке  промиса res
